...
void state_init() {
    ...
    pthread_mutex_init(&freeinode,NULL);
    pthread_mutex_init(&freeBlocks,NULL);
    pthread_mutex_init(&open_file,NULL);

}

void state_destroy() { /* nothing to do */
    pthread_mutex_destroy(&freeinode);
    pthread_mutex_destroy(&freeBlocks);
    pthread_mutex_destroy(&open_file);
}
    ...
    for (int inumber = 0; inumber < INODE_TABLE_SIZE; inumber++) {
        ...
        if (freeinode_ts[inumber] == FREE) {
            ...
            if (b == -1) {
                ...
                pthread_mutex_unlock(&freeinode);
                return -1;
            }
            ...
            if (n_type == T_DIRECTORY) {
                ...
                for (int i = 0; i < 10; i++){
                    inode_table[inumber].d_data_block[i] = -1;
                }
                ...
                int* block = (int*)data_block_get(b);
...
int inode_delete(int inumber) {
    ...
    if (inode_table[inumber].i_size > 0) {
        int* block = (int*)data_block_get(inode_table[inumber].i_data_block);
        int amount = (int)((inode_table[inumber].i_size)/BLOCK_SIZE + 1);
        for (int i = 0; i < amount; i++){
            if (i >= 10){
                if (data_block_free(block[i-10]) == -1){
                    return -1;
                }
            }
            else if(inode_table[inumber].d_data_block[i] > -1){
                if (data_block_free(inode_table[inumber].d_data_block[i]) == -1){
                    return -1;
                }
            }
        }
...
int data_block_alloc() {
    ...
    for (int i = 0; i < DATA_BLOCKS; i++) {
        ...
        if (free_blocks[i] == FREE) {
            free_blocks[i] = TAKEN;
            pthread_mutex_unlock(&freeBlocks);
            return i;
        }
    }
    pthread_mutex_unlock(&freeBlocks);
...
int add_to_open_file_table(int inumber, size_t offset) {
    ...
    pthread_mutex_lock(&open_file);
    for (int i = 0; i < MAX_OPEN_FILES; i++) {
        if (free_open_file_entries[i] == FREE) {
            free_open_file_entries[i] = TAKEN;
            open_file_table[i].of_inumber = inumber;
            open_file_table[i].of_offset = offset;
            pthread_mutex_init(&open_file_table[i].file_lock,NULL);
            pthread_mutex_unlock(&open_file);
            return i;
        }
    }
    pthread_mutex_unlock(&open_file);
...
int remove_from_open_file_table(int fhandle) {
    ...
    pthread_mutex_destroy(&open_file_table[fhandle].file_lock);
    return 0;
}
