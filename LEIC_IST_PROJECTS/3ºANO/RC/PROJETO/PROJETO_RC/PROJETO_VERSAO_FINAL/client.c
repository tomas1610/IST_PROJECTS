// Trabalho realizado por Tomás Marques 99340
...
void start_request(char PLID[], char command[]){

    char status[4],buffer[MAX_BUF_LEN], code[6];
    int i, args;
     
    args = sscanf(command,"%s %s",code,PLID);
    if (args != 2){
...
void play_request(char PLID[], char command[]){

    char status[4],buffer[MAX_BUF_LEN], code[5];
    int offset, n_pos, i, pos,pos_aux,trial,count,aux, args;
    char letter;

    args = sscanf(command,"%s %c",code,&letter);
    ...
    if (isalpha(letter) == 0){
        printf("Argument is not a letter\n");
        return;
    }
...
void guess_request(char PLID[], char command[]){

    char word_guess[31], status[4],buffer[MAX_BUF_LEN], code[6];
    int args, i;

    args = sscanf(command,"%s %s",code, word_guess);
    if (args != 2){
        printf("Number of arguments invalid\n");
        return;
    }

    for (i = 0; i< strlen(word_guess); i++){
        if (isalpha(word_guess[i]) == 0){
...
void rev_request(char PLID[]){
    ...
    if (!strcmp(word_rev,"ERR")){
        ...
        printf("GAME OVER: your word was %s\n",word_rev);
    }
...
void handle_scoreboard(char PLID[]){
    ...
    time.tv_sec = 10;
    time.tv_usec = 0;
    if (setsockopt(tcp_fd,SOL_SOCKET,SO_RCVTIMEO,&time,sizeof(time)) < 0)
        exit(1);
...
int handle_request(){
    ...
    char command[11], PLID[7], buffer[MAX_BUF_LEN];
